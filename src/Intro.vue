        ...
        <div v-if="gardens.length > 0">
          ...
          <div v-if="gardenPlants.length > 0">
            ...
            <p>
              ...
              on the indvidual garden plant page. There's two ways to get to that page. On the
              <router-link v-bind:to="{name: 'home'}">home</router-link>page,
              there's a list of currently growing plants. You can click on the plant name, and you can go to
              the individual garden plant page. The other option is to go to
              <router-link v-bind:to="{name: 'gardens'}">Your Gardens</router-link>
              ,
              then click the garden that has the plant in it, then click the individual garden plant. For this tutorial, you
              can look at the {{gardenPlants[0].name}} harvests by going to the
              <router-link
                :to="{ name: 'gardenplant', params: { id: gardenPlants[0].id }}"
              >{{gardenPlants[0].name}}</router-link>&nbsp;page.
            </p>
            <p>
              ...
              The sum total amount you harvested is rolled up and displayed on the
              <router-link v-bind:to="{name: 'home'}">home</router-link>&nbsp;page
            ...
            <p>
              Try adding a harvest for your {{gardenPlants[0].name}} plant by clicking the "Add a new Harvest" button below!
              </p>
            <add-harvest-component
              v-bind:uri="harvestURI"
              v-bind:gardenPlant="gardenPlants[0]"
...
import addHarvestComponent from "./components/GardenPlant/addHarvestComponent";
...
export default {
  ...
  data() {
    return {
      ...
      harvestURI: config.apiURL + "/api/GardenPlantHarvests",
      plantGroups: [],
      plants: [],
      gardens: [],
      garden: {},
  ...
  components: {
    ...
    "add-harvest-component": addHarvestComponent
  },
  methods: {
    GetContent: function() {
